...
from manipulathor_baselines.stretch_bring_object_baselines.models.stretch_object_displacement_map import StretchObjectDisplacementMapModel
...
from scripts.dataset_generation.find_categories_to_use import FULL_LIST_OF_OBJECTS, KITCHEN_TRAIN, LIVING_ROOM_TRAIN, \
    ...
    KITCHEN_TEST, KITCHEN_VAL, BEDROOM_VAL, LIVING_ROOM_VAL, BATHROOM_VAL
...
class PointNavEmulStretchRobothorOnlyPickupNoNoiseFullFovWithMap(
    StretchBringObjectiThorBaseConfig,
    StretchBringObjectMixInPPOConfig,
    StretchBringObjectMixInSimpleGRUConfig,
):
    ...
    source_mask_sensor_intel = IntelNoisyObjectMask(height=desired_screen_size, width=desired_screen_size,noise=0, full_frame=True, type='source', distance_thr=distance_thr, only_close_big_masks=True)
    destination_mask_sensor_intel = IntelNoisyObjectMask(height=desired_screen_size, width=desired_screen_size,noise=0, full_frame=True, type='destination', distance_thr=distance_thr, only_close_big_masks=True)
    depth_sensor_intel = IntelRawDepthSensor(full_frame=True)

    source_mask_sensor_kinect = KinectNoisyObjectMask(height=desired_screen_size, width=desired_screen_size,noise=0, full_frame=True, type='source', distance_thr=distance_thr, only_close_big_masks=True)
    destination_mask_sensor_kinect = KinectNoisyObjectMask(height=desired_screen_size, width=desired_screen_size,noise=0, full_frame=True, type='destination', distance_thr=distance_thr, only_close_big_masks=True)
    ...
    SENSORS = [
        ...
        PrevFrameSensor(
            RGBSensorStretchIntel(
                ...
                uuid="rgb_lowres_prev",
            ),
            uuid="rgb_lowres_prev_frame",
        ),
        RGBSensorStretchIntel(
            height=desired_screen_size,
            width=desired_screen_size,
    ...
    VALID_SCENES = ROBOTHOR_VAL
    VAL_SCENES = ROBOTHOR_VAL
    ...
    def test_task_sampler_args(self, **kwargs):
        sampler_args = super(PointNavEmulStretchRobothorOnlyPickupNoNoiseFullFovWithMap, self).test_task_sampler_args(**kwargs)
    ...
    def train_task_sampler_args(self, **kwargs):
        sampler_args = super(PointNavEmulStretchRobothorOnlyPickupNoNoiseFullFovWithMap, self).train_task_sampler_args(**kwargs)
        if platform.system() == "Darwin":
            pass
        else:
    ...
    def create_model(cls, **kwargs) -> nn.Module:
        return StretchObjectDisplacementMapModel(
